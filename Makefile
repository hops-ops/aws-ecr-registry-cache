...
PACKAGE ?= aws-ecr-registry-cache
XRD_DIR := apis/ecrregistrycaches
COMPOSITION := $(XRD_DIR)/composition.yaml
DEFINITION := $(XRD_DIR)/definition.yaml
CONFIGURATION := $(XRD_DIR)/configuration.yaml
EXAMPLE_DEFAULT := examples/ecrregistrycaches/standard.yaml
...
EXAMPLES := \
    examples/ecrregistrycaches/standard.yaml:: \
    examples/ecrregistrycaches/authenticated-upstreams.yaml:: \
    examples/ecrregistrycaches/enhanced-scanning.yaml::
...
render\:%:
	@example="examples/ecrregistrycaches/$$*.yaml"; \
	if [ -f "$$example" ]; then \
		echo "=== Rendering $$example ==="; \
		up composition render --xrd=$(DEFINITION) $(COMPOSITION) $$example; \
	else \
